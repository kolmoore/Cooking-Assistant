# Code was created with the assistance of AI
...
class SmartKitchenAssistantApp(tk.Tk):
    def __init__(self):
        """Initialize the Smart Kitchen Assistant application."""
        super().__init__()
        self.family_profiles = {}  # Dictionary to hold family profiles
        self.load_profiles()  # Load family profiles at startup

        # Initialize StringVar for various UI elements
        ...
        self.family_selected = 0  # Track the state of the selected family
        self.title("Smart Kitchen Assistant")  # Set the window title
    ...
    def show_welcome_screen(self):
        """Display the Welcome Screen with family selection options."""
        # Clear any existing widgets from the main content area
        for widget in self.main_content_frame.winfo_children():
            widget.destroy()

        # Placeholder for the logo image
        logo_image = tk.PhotoImage(file="Project/HomeScreen.png")  # Update with the correct logo path
        ...
        # Family Dropdown for selecting family names
        ...
        self.family_dropdown['values'] = list(self.family_profiles.keys())  # Load family names into the dropdown
        self.family_dropdown.set("Family")  # Set default text
        ...
        if self.family_selected:
            ...
            self.update_family_dropdown()  # Update dropdown after loading profiles

        # Create a button frame for family selection actions
        ...
        # Create buttons for family actions
    ...
    def show_main_menu_screen(self):
        """Display the Main Menu Screen after selecting a family."""
        # Clear the main content area
        for widget in self.main_content_frame.winfo_children():
            widget.destroy()

        # Create Home and Edit Family buttons on the main menu
        ...
        # Label displaying the selected family name
        family_name_label = tk.Label(self.main_content_frame, text=f"Stirring Up Some Recipes for the {self.family_name} family!", font=("Helvetica", 16, "bold"))
        family_name_label.grid(row=1, column=0, columnspan=4, padx=10, pady=10)

        # Recipe Type Dropdown label and setup
        ...
        self.recipe_type_dropdown.set("Any")  # Set default value
        self.recipe_type_dropdown.grid(row=2, column=1, padx=10, pady=10)
        self.recipe_type_dropdown.bind("<<ComboboxSelected>>", lambda event: self.sort_recipes())  # Bind event to sort recipes on selection

        # Recipe Complexity Dropdown label and setup
        ...
        self.recipe_complexity_dropdown.set("Any")  # Set default value
        self.recipe_complexity_dropdown.grid(row=3, column=1, padx=10, pady=10)
        self.recipe_complexity_dropdown.bind("<<ComboboxSelected>>", lambda event: self.sort_recipes())  # Bind event to sort recipes on selection

        # Recipe Rating Dropdown label and setup
        ...
        self.recipe_rating_dropdown.set("Any")  # Set default value
        self.recipe_rating_dropdown.grid(row=2, column=3, padx=10, pady=10)
        self.recipe_rating_dropdown.bind("<<ComboboxSelected>>", lambda event: self.sort_recipes())  # Bind event to sort recipes on selection

        # Recipe Main Ingredient Dropdown label and setup
        ...
        self.recipe_main_ing_dropdown.set("Any")  # Set default value
        self.recipe_main_ing_dropdown.grid(row=3, column=3, padx=10, pady=10)
        self.recipe_main_ing_dropdown.bind("<<ComboboxSelected>>", lambda event: self.sort_recipes())  # Bind event to sort recipes on selection

        # Recipe List Label header
        ...
        # Create button frame for main menu actions
        ...
        # Create buttons for recipe actions
        get_recipe_button = tk.Button(self.mm_button_frame, text="Get Recipe", command=self.get_recipe, font=("Helvetica", 14), state=API)
        ...
        get_meal_plan_button = tk.Button(self.mm_button_frame, text="Get Meal Plan", command=self.get_meal_plan, font=("Helvetica", 14), state=API)
        get_meal_plan_button.grid(row=1, column=0, padx=15, pady=10)

        view_meal_plan_button = tk.Button(self.mm_button_frame, text="View Meal Plan", command=self.view_meal_plan, font=("Helvetica", 14))
        view_meal_plan_button.grid(row=1, column=1, padx=15, pady=10)
        ...
        # Populate the recipe list on startup
    ...
    def show_edit_profile_screen(self):
        ...
        # Create Home and Go Cook buttons
        home_button = tk.Button(self.main_content_frame, text="Home", command=self.show_welcome_screen, font=("Helvetica", 14))
        home_button.grid(row=0, column=0, padx=10, pady=10)
        ...
        # Member Name Dropdown label and setup
        ...
        self.populate_member_names_dropdown()  # Populate this dropdown with member names
        self.member_name_dropdown.set("All")  # Default to "All"
        self.member_name_dropdown.grid(row=2, column=1, padx=10, pady=10)
        self.member_name_dropdown.bind("<<ComboboxSelected>>", lambda event: self.filter_members())  # Bind event to filter members

        # Member Age Dropdown label and setup
        ...
        self.member_age_dropdown['values'] = age_ranges  # Set age ranges for selection
        self.member_age_dropdown.set("All")  # Default to "All"
        self.member_age_dropdown.grid(row=3, column=1, padx=10, pady=10)
        self.member_age_dropdown.bind("<<ComboboxSelected>>", lambda event: self.filter_members())  # Bind event to filter members

        # Dietary Restrictions Dropdown label and setup
        ...
        self.populate_dietary_restrictions_dropdown()  # Populate this dropdown with dietary restrictions
        ...
        self.dietary_restrictions_dropdown.bind("<<ComboboxSelected>>", lambda event: self.filter_members())  # Bind event to filter members

        # Favorite Foods Dropdown label and setup
        ...
        self.populate_favorite_foods_dropdown()  # Populate this dropdown with favorite foods
        self.favorite_foods_dropdown.set("All")  # Default to "All"
        self.favorite_foods_dropdown.grid(row=3, column=3, padx=10, pady=10)
        self.favorite_foods_dropdown.bind("<<ComboboxSelected>>", lambda event: self.filter_members())  # Bind event to filter members

        # Member List Label header
        ...
        # Create button frame for editing members
        ...
        # Populate the member list with family members
    ...
    def filter_members(self):
        ...
        if family_name in self.family_profiles:
            ...
            if selected_age_range != "All":
                ...
                min_age, max_age = age_ranges[selected_age_range]  # Get min and max age from selected range
    ...
    def populate_recipe_list(self):
        ...
        self.populate_dropdowns()  # Populate dropdowns with unique values
        ...
        if family_name in self.family_profiles:
            ...
            for recipe in recipes:
                title = recipe['title'][:30]  # Take the first 30 characters of the title
    ...
    def populate_member_list(self):
        """Populate the member listbox with family members."""
        # Clear the current entries in the listbox
        self.member_listbox.delete(0, tk.END)
    ...
    def populate_dropdowns(self):
        """Populate the dropdown menus with unique values from the family's recipes."""
        ...
        # Convert sets to sorted lists and update dropdown values
    ...
    def get_recipe(self):
        """Get a recipe based on family member preferences and selected meal type."""
        ...
        if self.family_name and member_info:
            ...
            for member in member_info:
                ...
                ages_list.append(str(member.get('age', '')))  # Use get to avoid KeyError

            # Join the lists into a single string
            favorite_foods = ', '.join(favorite_foods_list) if favorite_foods_list else "None"
            dietary_restrictions = ', '.join(dietary_restrictions_list) if dietary_restrictions_list else "None"
            ages = ', '.join(ages_list)
            ...
            prompt = (f"Please provide a {recipe_type} recipe that uses {main_ingredient} as a main ingredient and is {complexity} difficulty. "
                      f"This recipe needs to feed a family of {num_members} and should incorporate their favorite foods: {favorite_foods} when possible. "
    ...
    def remove_recipe(self):
        ...
        if selected_index:
            ...
            recipe_title = selected_recipe_info[0:30].strip()  # Extract the title (first 30 characters)
    ...
    def request_recipe_from_api(self, prompt):
        ...
        try:
            ...
            # Parse and add the recipe to family profiles
    ...
    def parse_and_add_recipe(self, recipe_response):
        ...
        # Assuming the recipe_response is a string formatted as a dictionary
        ...
        try:
            ...
            ingredients = recipe_dict['ingredients']  # Already a list of dictionaries
            instructions = recipe_dict['instructions']  # Already a list of strings
            ...
            if family_name in self.family_profiles:
                ...
                self.populate_recipe_list()  # Refresh the recipe list
    ...
    def make_recipe(self):
        ...
        if selected_index:
            ...
            recipe_title = selected_recipe_info[0:30].strip()  # Assuming the title is the first 30 characters
    ...
    def sort_recipes(self):
        """Sort the recipes based on selected criteria from dropdowns."""
        recipe_type = self.recipe_type_variable.get()
    ...
    def get_meal_plan(self):
        """Show the Meal Plan selection popup to create a new meal plan."""
        meal_plan_window = tk.Toplevel(self)
        meal_plan_window.title("Create Meal Plan")

        # Meal Type Dropdowns for selecting meal types
        ...
        # Family Member Dropdown for selecting a family member
        ...
        # Meal Plan Keywords entry for additional filtering
        ...
        # OK Button to create the meal plan
    ...
    def load_profiles(self):
        """Load family profiles from a JSON file."""
        if os.path.exists("Project/profiles.json"):
            with open("Project/profiles.json", "r") as f:
                profiles_data = json.load(f)
                self.family_profiles = profiles_data

    def save_profiles(self):
        """Save family profiles to a JSON file."""
        profiles_data = self.family_profiles
        with open("Project/profiles.json", "w") as f:
    ...
    def edit_profile(self):
        ...
        if self.family_name in self.family_profiles:
            self.show_edit_profile_screen()  # Navigate to the edit profile screen
        else:
            messagebox.showerror("Error", "Please select a valid family.")

    def add_family(self):
        """Add a new family profile to the application."""
        family_name = simpledialog.askstring("Add Family", "Enter family name:")
        if family_name and family_name not in self.family_profiles:
            self.family_profiles[family_name] = {
                "members": [],  # Initialize with no members
                "kitchens": {},  # Initialize kitchens as empty
            ...
            self.update_family_dropdown()  # Update dropdowns with the new family
    ...
    def remove_family(self):
        """Remove the selected family profile from the application."""
        family_name = self.family_name_variable.get()
    ...
    def add_dietary_restriction(self, member_info):
        """Add a selected dietary restriction to the member's profile."""
    ...
    def add_custom_dietary_restriction(self, member_info):
        """Add a custom dietary restriction to the member's profile."""
    ...
    def remove_dietary_restriction(self, member_info):
        """Remove the selected dietary restriction from the member's profile."""
        restriction = self.dietary_restrictions_dropdown.get()
    ...
    def add_favorite_food(self, member_info):
        """Add a selected favorite food to the member's profile."""
    ...
    def add_custom_favorite_food(self, member_info):
        """Add a custom favorite food to the member's profile."""
    ...
    def remove_favorite_food(self, member_info):
        """Remove the selected favorite food from the member's profile."""
    ...
    def save_member(self, member_info, window):
        """Save the updated information of a family member."""
        ...
        self.populate_member_list()  # Refresh the member list
    ...
    def add_member(self):
        ...
        if member_name:
            if member_name not in [member['name'] for member in self.family_profiles[self.family_name]["members"]]:
                member_info = {
                    "dietary_restrictions": [],  # Initialize with no dietary restrictions
                    "favorite_foods": [],  # Initialize with no favorite foods
                    "name": member_name,
                    "age": ""  # Initialize age as empty
                }

                self.family_profiles[self.family_name]["members"].append(member_info)
                self.save_profiles()  # Save the profiles
                self.populate_member_list()  # Update listbox with new member
    ...
    def edit_member(self):
        ...
        if selected_member_index:
            ...
            if member_info:
                ...
                self.dietary_restrictions_dropdown['values'] = ["All"] + member_info['dietary_restrictions'] + ["Dairy Free", "Gluten Free", "Vegetarian", "Vegan"]  # Prepopulate with the user's dietary restrictions
                self.dietary_restrictions_dropdown.set("All")  # Default to "All"
    ...
    def remove_member(self):
        """Remove the selected family member from the family's profile."""
        family_name = self.family_name_variable.get()
        selected_member_index = self.member_listbox.curselection()  # Get the index of the selected member
